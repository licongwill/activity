...
public class RedisSessionDAO extends AbstractSessionDAO {
	...
	private void saveSession(Session session) throws UnknownSessionException {
		...
		if (expire == DEFAULT_EXPIRE) {
			this.ssoRedisManager.set(key, value, (int) (session.getTimeout() / MILLISECONDS_IN_A_SECOND));
			return;
		}
		...
		this.ssoRedisManager.set(key, value, expire);
	}
	...
	public void delete(Session session) {
		...
		try {
			this.ssoRedisManager.del(keySerializer.serialize(getRedisSessionKey(session.getId())));
		} catch (SerializationException e) {
	...
	public Collection<Session> getActiveSessions() {
		...
		try {
			Set<byte[]> keys = this.ssoRedisManager.keys(this.keySerializer.serialize(this.keyPrefix + "*"));
			if (keys != null && keys.size() > 0) {
				for (byte[] key:keys) {
					Session s = (Session) valueSerializer.deserialize(this.ssoRedisManager.get(key));
					sessions.add(s);
				}
			}
		} catch (SerializationException e) {
	...
	protected Session doReadSession(Serializable sessionId) {
		...
		try {
			session = (Session) valueSerializer.deserialize(this.ssoRedisManager.get(keySerializer.serialize(getRedisSessionKey(sessionId))));
			if (this.sessionInMemoryEnabled) {
	...
	public SsoRedisManager getSsoRedisManager() {
		return ssoRedisManager;
	}

	public void setSsoRedisManager(SsoRedisManager ssoRedisManager) {
